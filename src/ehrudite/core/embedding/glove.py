"""Glove embedding from https://github.com/GradySimon/tensorflow-glove"""
...
class GloVeModel:
            ...
            for epoch in range(num_epochs):
                ...
                for batch_index, batch in enumerate(batches):
                    ...
                    if (
                        ...
                        and (total_steps + 1) % summary_batch_interval == 0
                    ):
    ...
    def __fit_to_corpus(
        self, corpus, vocab_size, min_occurrences, left_size, right_size
    ):
        word_counts = Counter()
        cooccurrence_counts = defaultdict(float)
        for region in corpus:
            word_counts.update(region)
            for l_context, word, r_context in _context_windows(
                region, left_size, right_size
            ):
    ...
    def __build_graph(self):
        ...
        with self.__graph.as_default(), self.__graph.device(_device_for_node):
            count_max = tf.constant(
                [self.cooccurrence_cap], dtype=tf.float32, name="max_cooccurrence_count"
            )
            scaling_factor = tf.constant(
                [self.scaling_factor], dtype=tf.float32, name="scaling_factor"
            )

            self.__focal_input = tf.compat.v1.placeholder(
                tf.int32, shape=[self.batch_size], name="focal_words"
            )
            self.__context_input = tf.compat.v1.placeholder(
                tf.int32, shape=[self.batch_size], name="context_words"
            )
            self.__cooccurrence_count = tf.compat.v1.placeholder(
            ...
            weighting_factor = tf.minimum(
                1.0,
                tf.pow(tf.divide(self.__cooccurrence_count, count_max), scaling_factor),
            )
            ...
            log_cooccurrences = tf.math.log(
                tf.cast(self.__cooccurrence_count, tf.float32)
            )
            ...
            self.__optimizer = tf.compat.v1.train.AdagradOptimizer(
                self.learning_rate
            ).minimize(self.__total_loss)
            self.__summary = tf.compat.v1.summary.merge_all()

            self.__combined_embeddings = tf.add(
                focal_embeddings, context_embeddings, name="combined_embeddings"
            )
